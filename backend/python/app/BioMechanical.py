...
# ============================================================================
# ANALYSES UNIFIÉES - FIGURES STATIQUES
# ============================================================================
...
def analyze_static():
    ...
    try:
        ...
        with mp_pose.Pose(static_image_mode=True, min_detection_confidence=0.7) as pose:
            ...
            user_angles = calculate_user_angles(results.pose_landmarks, image.shape)
...
@app.route("/analyze_video_dynamic", methods=["POST"])
def analyze_video_dynamic():
    """Analyse vidéo pour mouvements DYNAMIQUES (push-ups, pull-ups, dips)."""
    logger.info("=== ANALYSE VIDÉO DYNAMIQUE ===")

    try:
        if 'file' not in request.files:
            return jsonify({"status": "error", "message": "Aucun fichier vidéo"}), 400
        ...
        if not cap.isOpened():
            return jsonify({"status": "error", "message": "Impossible d'ouvrir la vidéo"}), 400

        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        output_path = os.path.join(UPLOAD_FOLDER, "annotated_" + video_file.filename)
        ...
        with mp_pose.Pose(
            ...
            while cap.isOpened():
                ...
                if results.pose_landmarks:
                    ...
                    angles = calculate_user_angles(results.pose_landmarks, frame.shape)
        ...
        if len(landmarks_sequence) < 10:
            return jsonify({"status": "error", "message": "Pas assez de données"}), 400
        ...
        if movement_type != "dynamic":
            return jsonify({
                "status": "error",
                "message": f"Ce n'est pas un mouvement dynamique, mais plutôt : {movement_type}",
                "detected_type": movement_type,
                "detected_figure": figure,
                "suggestion": "Utilise /analyze_video_static pour ce type de mouvement"
            }), 400

        logger.info(f"Exercice dynamique détecté: {figure}")

        # Analyse selon l'exercice
        if figure == "push_up":
            signal = [(a.get("left_elbow", 0) + a.get("right_elbow", 0)) / 2 for a in angles_sequence]
            model = dynamic_skills["push_up"]
            cycles = extract_cycles_generic(signal, timestamps, model)
            analysis = analyze_push_up(cycles, model)

        elif figure == "pull_up":
            signal = [(a.get("left_elbow", 0) + a.get("right_elbow", 0)) / 2 for a in angles_sequence]
            model = dynamic_skills["pull_up"]
            cycles = extract_cycles_generic(signal, timestamps, model, signal_inverted=True)
            analysis = analyze_pull_up(cycles, model)

        elif figure == "dips":
            signal = [(a.get("left_elbow", 0) + a.get("right_elbow", 0)) / 2 for a in angles_sequence]
            model = dynamic_skills["dips"]
            cycles = extract_cycles_generic(signal, timestamps, model)
            ...
            return jsonify({
                "status": "error",
                "message": f"Exercice dynamique non reconnu: {figure}"
            }), 400
        ...
        if cycles:
            ...
            avg_amplitude = np.mean([c["amplitude"] for c in cycles])
            avg_duration = np.mean([c["duration"] for c in cycles])
            total_reps = len(cycles)
        else:
            avg_min = avg_max = avg_amplitude = avg_duration = 0
            total_reps = 0
        ...
        return jsonify({
            ...
            "message": "Analyse dynamique terminée",
            ...
            "user_angles": {
                ...
                "average_amplitude": round(avg_amplitude, 1),
        ...
        logger.error(f"Erreur dans analyze_video_dynamic: {e}", exc_info=True)
        return jsonify({"status": "error", "message": f"Erreur: {str(e)}"}), 500


@app.route("/analyze_video_static", methods=["POST"])
def analyze_video_static():
    """Analyse vidéo pour figures STATIQUES tenues (handstand, planche, front_lever)."""
    logger.info("=== ANALYSE VIDÉO STATIQUE ===")
    ...
    try:
        ...
        if movement_type != "static":
            return jsonify({
                "status": "error",
                "message": f"Ce n'est pas une figure statique, mais plutôt : {movement_type}",
                "detected_type": movement_type,
                "detected_figure": figure,
                "suggestion": "Utilise /analyze_video_dynamic pour ce type de mouvement"
            }), 400

        logger.info(f"Figure statique détectée: {figure}")

        # Analyse unifiée de la figure statique
        if figure in static_skills:
            model = static_skills[figure]
            analysis = analyze_static_hold_unified(figure, angles_sequence, model)
        else:
            return jsonify({
                "status": "error",
                "message": f"Figure statique non reconnue: {figure}"
            }), 400

        # Durée totale de maintien
        ...
        logger.error(f"Erreur dans analyze_video_static: {e}", exc_info=True)
        return jsonify({"status": "error", "message": f"Erreur: {str(e)}"}), 500
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Serveur d'analyse de mouvement - ANALYSE UNIFIÉE")
    logger.info("Version: 7.0")
    logger.info("Exercices supportés:")
    logger.info("  Dynamiques (vidéo): Push-ups, Pull-ups, Dips")
    logger.info("  Statiques (image + vidéo): Handstand, Planche, Front Lever")
    logger.info("Routes:")
    logger.info("  /analyze_static → analyse image")
    logger.info("  /analyze_video_dynamic → analyse mouvements répétitifs")
    logger.info("  /analyze_video_static → analyse figures tenues")
